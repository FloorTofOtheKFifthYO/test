...
void lcd_init(){
  ...
  GPIO_Configuration(GPIO_Pin_12,GPIO_Mode_AF, GPIO_OType_PP,GPIO_Speed_50MHz,GPIO_PuPd_NOPULL,GPIOF);
	...
	if(lcd_id==0X9300 || lcd_id == 0){//因为9341在未被复位的情况下会被读成9300	
 		//尝试9341 ID的读取		
		lcd_wr_reg(0XD3);				   
		lcd_id=lcd_rd_data();	//dummy read 	
 		lcd_id=lcd_rd_data();	//读到0X00
  	lcd_id=lcd_rd_data();   	//读取93								   
 		lcd_id<<=8;
		lcd_id|=lcd_rd_data();  	//读取41 	   			   
    ...
    if(lcd_id == 0x9325){
						...
						lcd_reg_write(0x0003,0x1020);   //渲染方向0x1018
		...
		GPIO_SetBits(GPIOD,GPIO_Pin_10);  //背光亮起
		g_lcd_dir = LCD_DIR;
